...
class DeleteStorage(BatchFilter):
    """
    ...
    def description(self) -> str:
        ...
        :rtype: str
        """
        return "Removes the specified object from storage whenever data passes through."
    ...
    def _create_argparser(self) -> argparse.ArgumentParser:
        ...
        """
        parser = super()._create_argparser()
        parser.add_argument("-s", "--storage_name", type=str, help="The name of the object in internal storage to remove.", required=True)
        return parser
    ...
    def _do_process(self, data):
        ...
        if self.storage_name in self.session.storage:
            self.logger().info("Removing from storage: %s" % self.storage_name)
            del self.session.storage[self.storage_name]
        return data
