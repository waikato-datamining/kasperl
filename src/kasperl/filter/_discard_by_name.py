...
class DiscardByName(Filter):
    ...
    def description(self) -> str:
        ...
        :rtype: str
        """
        return "Discards files based on list of names, list of paths and/or regular expressions that names must match."
    ...
    def _create_argparser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-i", "--names", type=str, help="The name(s) to drop.", required=False, nargs="*")
        parser.add_argument("-I", "--names_file", type=str, help="The text file with the name(s) to drop.", required=False, default=None)
        parser.add_argument("-p", "--paths", type=str, help="The directories with files to ignore.", required=False, nargs="*")
        parser.add_argument("-r", "--regexps", type=str, help="The regular expressions for matching name(s) to drop.", required=False, nargs="*")
        parser.add_argument("-R", "--regexps_file", type=str, help="The text file with regular expressions for matching name(s) to drop.", required=False, default=None)
        ...
        for item in make_list(data):
            ...
            if len(self._names) > 0:
                if image_name in self._names:
                    if not self.invert:
                        self.logger().info("Skipping based on name match: %s" % item.get_name())
                        add = False
                else:
                    if self.invert:
                        self.logger().info("Skipping based on no name match (invert): %s" % item.get_name())
                        add = False
            ...
            if add:
                for regexp in self._regexps:
                    if regexp.fullmatch(image_name) is not None:
                        if not self.invert:
                            self.logger().info("Skipping based on regexp match: %s" % item.get_name())
                            add = False
                            break
                    else:
                        if self.invert:
                            self.logger().info("Skipping based on no regexp match (invert): %s" % item.get_name())
                            add = False
                            break
