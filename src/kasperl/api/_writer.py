...
def parse_writer(writer: str, available_writers: Dict[str, Plugin]) -> seppl.io.Writer:
    """
    Parses the command-line and instantiates the writer.

    :param writer: the command-line to parse
    :type writer: str
    :param available_writers: the writers to use for parsing
    :type available_writers: dict
    :return: the writer
...
def add_annotations_only_writer_param(parser: argparse.ArgumentParser):
    ...
    """
    parser.add_argument("--annotations_only", action="store_true", help="Outputs only the annotations.")
